...
class Mesh
{
	...
	protected :
		...
		std::vector<Vector3i> facetex_index;

		std::vector<Vector3i> facenormal_index;
	...
	public :
		...
		inline Mesh(const Mesh& m) : vertices(m.vertices), faces(m.faces),
		...
		// Add face f index to face array
		inline void AddFaceNormalIndex(const Vector3i& f) {
			facenormal_index.push_back(f);
		}

		// Add face f to face array
		inline void AddFaceCoordIndex(const Vector3i& f) {
			facetex_index.push_back(f);
		}
